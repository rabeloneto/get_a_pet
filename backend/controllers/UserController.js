...
module.exports = class UserController {
  static async register(req, res) {
    ...
    if (!email) {
      res.status(422).json({ message: " O email é necessario" });
      return;
    }
    if (!phone) {
    ...
    if (!password) {
      res.status(422).json({ message: " a senha e necessaria" });
      return;
    }
    if (!confirmpassword) {
      res.status(422).json({ message: " a confirmação de senha é necessaria" });
      return;
    }
    ...
    if (password !== confirmpassword) {
      res
        .status(422)
        .json({ message: "a senha e a confirmacao de senha precisam bater" });
      return;
    }
    ...
    if (userExists) {
      res.status(422).json({ message: "Email já usado" });
      return;
    }
  ...
  static async editUser(req, res) {
    ...
    if (!phone) {
      res.status(422).json({ message: " O phone é necessario" });
      return;
    }
