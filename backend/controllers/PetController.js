...
module.exports = class PetController{
    ...
    static async create(req,res){
        ...
        if(!age){
            res.status(422).json({message: 'idade e obrigatorio'})
            return 
        }
        if(!weight){
    ...
    static async removePetById(req,res){
        ...
        //check if logged user register the pet
        const token = getToken(req)
        const user =await  getUserByToken(token)
    ...
    static async schedule(req,res){
        ...
        const user = await getUserByToken(token)
         ...
         if(pet.user._id.equals(user._id)){
          res.status(422).json({message: 'Nao pode agenndar visita ao proprio pet!'})
          return
        }
        ...
        if(pet.adopter){
            if(pet.adopter._id.equals(user._id)){
             res.status(422).json({message: 'voce ja agendou uma visita para esse pet!'})
             return
            }
        }
        ...
        await Pet.findByIdAndUpdate(id,pet)
         res.status(200).json({
            message: `visita agendada, entre em contato com ${pet.user.name} pelo telefone ${pet.user.phone}`,
        })
    }
    static async concludeAdoption(req,res){
        ...
        res.status(200).json({message: 'Parabens! adotado com sucesso'})
